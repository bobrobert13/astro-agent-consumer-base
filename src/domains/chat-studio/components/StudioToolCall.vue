...
/**
 * @file src/domains/chat-studio/components/StudioToolCall.vue
 * @description Tarjeta de una herramienta invocada durante la ejecución.
 *
 * Se carga con `defineAsyncComponent` desde `StudioMessage`: en la mayoría de las
 * respuestas no hay tool-calls, y su DOM (con el JSON formateado) no debe estar
 * en el chunk inicial del transcript.
 *
 ...
 * el mismo motivo que en `StudioMarkdown` — un tool-result es entrada no
 * confiable.
 */
...
<template>
  <Collapsible v-model:open="open" class="not-prose my-2 rounded-panel border border-line bg-surface">
