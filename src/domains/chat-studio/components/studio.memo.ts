/**
 * @file src/domains/chat-studio/components/studio.memo.ts
 ...
 * vuelo llega **sintetizado** desde `StudioThread` con `id: 'streaming'` y
 ...
 * chunk hasta que la respuesta cierra y se promociona a mensaje real. El
 * resultado final llega bien —por eso el smoke de Electron no lo ve—, pero la
 * persona ve un chat parado.
 *
 * Por eso la longitud del texto forma parte de las dependencias: es lo único que
 * distingue un frame del siguiente. Al ser funciones puras, el invariante se
 * prueba sin montar un componente.
 *
 * Se mudó aquí desde `agent-chat` al separar el motor de la vista: memorizar es
 * una decisión de render, no del transporte.
 */
import type { ChatMessage } from '@domains/agent-chat';
